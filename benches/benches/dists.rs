...
pub fn dist_benchmarks(c: &mut Criterion) {
    ...
    c.bench_function("DistType::Uniform 10 samples", |b| {
        ...
        b.iter(|| {
            let point = rng.random_range(50_000.0..1_000_000.0);

            let x = round_f64(rng.random_range(0.0..=point));
            let y = round_f64(rng.random_range(x..=point));
            let d = Dist {
                dist: DistType::Uniform { low: x, high: y },
                start: 0.0,
                max: 0.0,
            };
            bench_dist(&mut rng, d, black_box(n))
        })
    });
        ...
        b.iter(|| {
            let d = Dist {
                dist: DistType::Binomial {
                    trials: rng.random_range(10..=1000),
                    probability: round_f64(rng.random_range(0.0..=1.0)),
                },
                start: 0.0,
                max: 0.0,
            };
        ...
        b.iter(|| {
            ...
            let d = Dist {
                dist: DistType::Pareto {
                    scale: round_f64(
                        rng.random_range::<f64, _>(point / 100.0..point / 10.0)
                            .max(0.001),
                    ),
    ...
    c.bench_function("DistType::Gamma 10 samples", |b| {
        ...
        b.iter(|| {
            ...
            let d = Dist {
                dist: DistType::Gamma {
                    scale: round_f64(rng.random_range(0.001..=point)),
                    shape: round_f64(rng.random_range(0.001..=10.0)),
                },
                start: 0.0,
                max: 0.0,
    ...
    c.bench_function("DistType::Beta 10 samples", |b| {
        ...
        b.iter(|| {
            ...
            let d = Dist {
                dist: DistType::Beta {
                    alpha: round_f64(rng.random_range(0.0..=point)),
                    beta: round_f64(rng.random_range(0.0..=point)),
                },
                start: 0.0,
                max: 0.0,
            };
